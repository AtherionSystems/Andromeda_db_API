...
public class TaskAssignmentsController {
    ...
    public ResponseEntity<List<TaskAssignment>> getAssignmentsByTask(@PathVariable Long taskId) {
        // Filtramos las asignaciones que pertenecen a esta tarea específica
        List<TaskAssignment> assignments = taskAssignmentService.findAll().stream()
    ...
    public ResponseEntity<?> removeAssignment(@PathVariable Long taskId, @PathVariable Long userId) {
        Optional<TaskAssignment> assignment = taskAssignmentService.findAll().stream()
                .filter(a -> a.getTask().getId().equals(taskId) && a.getUser().getId().equals(userId))
                .findFirst();
