...
public class BotCommandHandler {
    ...
    /**
     * Entry point. Parses the raw message text and dispatches to a handler.
     * Returns null if the command is not recognised (bot stays silent).
     */
    @Transactional
    public String handle(String rawText) {
        ...
        // strip optional @BotUsername suffix from the command token
        String cmd = parts[0];
        if (cmd.contains("@")) {
        ...
        return switch (cmd.toLowerCase()) {
            ...
            // ── write ─────────────────────────────────────────────────────────
            case "/newproject"    -> handleNewProject(args);
            case "/newtask"       -> handleNewTask(args);
            case "/taskstatus"    -> handleTaskStatus(args);
            case "/taskpriority"  -> handleTaskPriority(args);
            case "/projectstatus" -> handleProjectStatus(args);
            case "/addmember"     -> handleAddMember(args);
            default               -> null;
        };
    }
    ...
    private String handleProject(String args) {
        ...
        return String.format(
                ...
                p.getId(), p.getName(),
                nvl(p.getStatus(), "—"),
                fmt(p.getStartDate()), fmt(p.getEndDate()),
    ...
    private String handleTask(String args) {
        ...
        Tasks t = opt.get();
        Project proj = t.getProject();   // safe: inside @Transactional
        return String.format(
                "Task #%d\n" +
                "Title:    %s\n" +
                "Project:  #%d %s\n" +
     ...
     * /newproject <name> [| description] [| status]
     */
    private String handleNewProject(String args) {
     ...
     * /newtask <projectId> | <title> [| priority] [| status]
     */
    private String handleNewTask(String args) {
        if (args.isBlank()) return "Usage: /newtask <projectId> | <title> [| priority] [| status]";

        String[] pipes = splitPipes(args, 4);
        Long projectId = parseLong(pipes[0]);
        if (projectId == null) return "Usage: /newtask <projectId> | <title> [| priority] [| status]";
        if (pipes.length < 2 || pipes[1].isEmpty()) return "Task title cannot be empty.";

        String title    = pipes[1];
        String priority = pipes.length > 2 ? pipes[2] : "medium";
        String status   = pipes.length > 3 ? pipes[3] : "todo";
        ...
        return String.format("Task created!\nID:       %d\nTitle:    %s\nProject:  #%d %s\nPriority: %s\nStatus:   %s",
     ...
     * /taskstatus <taskId> <status>
     */
    private String handleTaskStatus(String args) {
     ...
     * /taskpriority <taskId> <priority>
     */
    private String handleTaskPriority(String args) {
        String[] tokens = args.split("\\s+", 2);
     ...
     * /projectstatus <projectId> <status>
     */
    private String handleProjectStatus(String args) {
        String[] tokens = args.split("\\s+", 2);
     ...
     * /addmember <projectId> <userId> [role]
     */
    private String handleAddMember(String args) {
        ...
        return String.format("Member added!\nProject: #%d %s\nUser:    @%s\nRole:    %s",
                project.getId(), project.getName(),
                user.getUsername(), saved.getRole());
    }

    // ── helpers ───────────────────────────────────────────────────────────────

    /** Split on ' | ' (pipe with surrounding spaces), up to limit segments. */
